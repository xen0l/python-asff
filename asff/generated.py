...
class AwsCodeBuildProjectArtifactsDetails(ASFFBaseModel):
	"""
Information about the build artifacts for the CodeBuild project.

:param artifact_identifier: An identifier for the artifact definition.
:param encryption_disabled: Indicates whether to disable encryption on the artifact. Only valid when Type is S3.
:param location: Only used when Type is S3. The name of the S3 bucket where the artifact is located.
:param name: Only used when Type is S3. The name of the artifact. Used with NamepaceType and Path to determine the pattern for storing the artifact.
:param namespace_type: Only used when Type is S3. The value to use for the namespace. Used with Name and Path to determine the pattern for storing the artifact.
:param override_artifact_name: Whether the name specified in the buildspec file overrides the artifact name.
:param packaging: Only used when Type is S3. The type of output artifact to create.
:param path: Only used when Type is S3. The path to the artifact. Used with Name and NamespaceType to determine the pattern for storing the artifact.
:param type: The type of build artifact.

:return: AwsCodeBuildProjectArtifactsDetails object
	"""
	artifact_identifier: Optional[NonEmptyString]
	encryption_disabled: Optional[Boolean]
	location: Optional[NonEmptyString]
	name: Optional[NonEmptyString]
	namespace_type: Optional[NonEmptyString]
	override_artifact_name: Optional[Boolean]
	packaging: Optional[NonEmptyString]
	path: Optional[NonEmptyString]
	type: Optional[NonEmptyString]
AwsCodeBuildProjectArtifactsList = List[AwsCodeBuildProjectArtifactsDetails]
class AwsCodeBuildProjectEnvironmentEnvironmentVariablesDetails(ASFFBaseModel):
	"""
...
:return: AwsCodeBuildProjectEnvironmentEnvironmentVariablesDetails object
	"""
	name: Optional[NonEmptyString]
	type: Optional[NonEmptyString]
	value: Optional[NonEmptyString]
AwsCodeBuildProjectEnvironmentEnvironmentVariablesList = List[AwsCodeBuildProjectEnvironmentEnvironmentVariablesDetails]
...
:param environment_variables: A set of environment variables to make available to builds for the build project.
:param privileged_mode: Whether to allow the Docker daemon to run inside a Docker container. Set to true if the build project is used to build Docker images.
...
:return: AwsCodeBuildProjectEnvironment object
	...
	environment_variables: Optional[AwsCodeBuildProjectEnvironmentEnvironmentVariablesList]
	privileged_mode: Optional[Boolean]
...
class AwsCodeBuildProjectLogsConfigCloudWatchLogsDetails(ASFFBaseModel):
	"""
Information about CloudWatch Logs for the build project.

:param group_name: The group name of the logs in CloudWatch Logs.
:param status: The current status of the logs in CloudWatch Logs for a build project.
:param stream_name: The prefix of the stream name of the CloudWatch Logs.

:return: AwsCodeBuildProjectLogsConfigCloudWatchLogsDetails object
	"""
	group_name: Optional[NonEmptyString]
	status: Optional[NonEmptyString]
	stream_name: Optional[NonEmptyString]
class AwsCodeBuildProjectLogsConfigS3LogsDetails(ASFFBaseModel):
	"""
Information about logs built to an S3 bucket for a build project.

:param encryption_disabled: Whether to disable encryption of the S3 build log output.
:param location: The ARN of the S3 bucket and the path prefix for S3 logs.
:param status: The current status of the S3 build logs.

:return: AwsCodeBuildProjectLogsConfigS3LogsDetails object
	"""
	encryption_disabled: Optional[Boolean]
	location: Optional[NonEmptyString]
	status: Optional[NonEmptyString]
class AwsCodeBuildProjectLogsConfigDetails(ASFFBaseModel):
	"""
Information about logs for the build project.

:param cloud_watch_logs: Information about CloudWatch Logs for the build project.
:param s3_logs: Information about logs built to an S3 bucket for a build project.

:return: AwsCodeBuildProjectLogsConfigDetails object
	"""
	cloud_watch_logs: Optional[AwsCodeBuildProjectLogsConfigCloudWatchLogsDetails]
...
:param artifacts: Information about the build artifacts for the CodeBuild project.
...
:return: AwsCodeBuildProjectDetails object
	...
	artifacts: Optional[AwsCodeBuildProjectArtifactsList]
	...
	logs_config: Optional[AwsCodeBuildProjectLogsConfigDetails]
...
class AwsCloudFrontDistributionViewerCertificate(ASFFBaseModel):
	"""
Provides information about the TLS/SSL configuration that the distribution uses to communicate with viewers.

:param acm_certificate_arn: The ARN of the ACM certificate. Used if the certificate is stored in ACM. If you provide an ACM certificate ARN, you must also provide MinimumCertificateVersion and SslSupportMethod.
:param certificate: The identifier of the certificate. Note that in CloudFront, this attribute is deprecated.
:param certificate_source: The source of the certificate identified by Certificate. Note that in CloudFront, this attribute is deprecated.
:param cloud_front_default_certificate: Whether the distribution uses the CloudFront domain name. If set to false, then you provide either AcmCertificateArn or IamCertificateId.
:param iam_certificate_id: The identifier of the IAM certificate. Used if the certificate is stored in IAM. If you provide IamCertificateId, then you also must provide MinimumProtocolVersion and SslSupportMethod.
:param minimum_protocol_version: The security policy that CloudFront uses for HTTPS connections with viewers. If SslSupportMethod is sni-only, then MinimumProtocolVersion must be TLSv1 or higher.
:param ssl_support_method: The viewers that the distribution accepts HTTPS connections from.

:return: AwsCloudFrontDistributionViewerCertificate object
	"""
	acm_certificate_arn: Optional[NonEmptyString]
	certificate: Optional[NonEmptyString]
	certificate_source: Optional[NonEmptyString]
	cloud_front_default_certificate: Optional[Boolean]
	iam_certificate_id: Optional[NonEmptyString]
	minimum_protocol_version: Optional[NonEmptyString]
	ssl_support_method: Optional[NonEmptyString]
...
:param viewer_certificate: Provides information about the TLS/SSL configuration that the distribution uses to communicate with viewers.
...
:return: AwsCloudFrontDistributionDetails object
	...
	viewer_certificate: Optional[AwsCloudFrontDistributionViewerCertificate]
	status: Optional[NonEmptyString]
...
:return: LoadBalancerState object
	...
	code: Optional[NonEmptyString]
	reason: Optional[NonEmptyString]
class AwsElbv2LoadBalancerAttribute(ASFFBaseModel):
	"""
A load balancer attribute.

:param key: The name of the load balancer attribute.
:param value: The value of the load balancer attribute.

:return: AwsElbv2LoadBalancerAttribute object
	"""
	key: Optional[NonEmptyString]
	value: Optional[NonEmptyString]
AwsElbv2LoadBalancerAttributes = List[AwsElbv2LoadBalancerAttribute]
...
:param load_balancer_attributes: Attributes of the load balancer.
...
:return: AwsElbv2LoadBalancerDetails object
	...
	type: Optional[NonEmptyString]
	vpc_id: Optional[NonEmptyString]
	load_balancer_attributes: Optional[AwsElbv2LoadBalancerAttributes]
...
:param tls_security_policy: The TLS security policy to apply to the HTTPS endpoint of the OpenSearch domain. Valid values:    Policy-Min-TLS-1-0-2019-07, which supports TLSv1.0 and higher    Policy-Min-TLS-1-2-2019-07, which only supports TLSv1.2  
...
:param zone_awareness_enabled: Whether to enable zone awareness for the Elasticsearch domain. When zone awareness is enabled, OpenSearch allocates the cluster's nodes and replica index shards across Availability Zones in the same Region. This prevents data loss and minimizes downtime if a node or data center fails.
...
:param index_slow_logs: Configures the OpenSearch index logs publishing.
:param search_slow_logs: Configures the OpenSearch search slow log publishing.
...
:param automated_update_date: The epoch time when the deployment window closes for required updates. After this time, Amazon OpenSearch Service schedules the software upgrade automatically.
...
:param elasticsearch_version: OpenSearch version.
:param elasticsearch_cluster_config: Information about an OpenSearch cluster configuration.
...
:param vpc_options: Information that OpenSearch derives based on VPCOptions for the domain.
...
:param owner_account_id: The Amazon Web Services account identifier of the account that owns the S3 bucket.
...
:return: AwsS3BucketDetails object
	...
	owner_account_id: Optional[NonEmptyString]
...
class AwsOpenSearchServiceDomainEncryptionAtRestOptionsDetails(ASFFBaseModel):
	"""
...
:return: AwsOpenSearchServiceDomainEncryptionAtRestOptionsDetails object
	"""
	enabled: Optional[Boolean]
	kms_key_id: Optional[NonEmptyString]
class AwsOpenSearchServiceDomainNodeToNodeEncryptionOptionsDetails(ASFFBaseModel):
	"""
Provides details about the configuration for node-to-node encryption.

:param enabled: Whether node-to-node encryption is enabled.

:return: AwsOpenSearchServiceDomainNodeToNodeEncryptionOptionsDetails object
	"""
	enabled: Optional[Boolean]
class AwsOpenSearchServiceDomainServiceSoftwareOptionsDetails(ASFFBaseModel):
	"""
Provides information about the state of the domain relative to the latest service software.

:param automated_update_date: The epoch time when the deployment window closes for required updates. After this time, OpenSearch Service schedules the software upgrade automatically.
:param cancellable: Whether a request to update the domain can be canceled.
:param current_version: The version of the service software that is currently installed on the domain.
:param description: A more detailed description of the service software status.
:param new_version: The most recent version of the service software.
:param update_available: Whether a service software update is available for the domain.
:param update_status: The status of the service software update.
:param optional_deployment: Whether the service software update is optional.

:return: AwsOpenSearchServiceDomainServiceSoftwareOptionsDetails object
	"""
	automated_update_date: Optional[NonEmptyString]
	cancellable: Optional[Boolean]
	current_version: Optional[NonEmptyString]
	...
	update_available: Optional[Boolean]
	update_status: Optional[NonEmptyString]
	optional_deployment: Optional[Boolean]
class AwsOpenSearchServiceDomainClusterConfigZoneAwarenessConfigDetails(ASFFBaseModel):
	"""
Configuration options for zone awareness.

:param availability_zone_count: The number of Availability Zones that the domain uses. Valid values are 2 and 3. The default is 2.

:return: AwsOpenSearchServiceDomainClusterConfigZoneAwarenessConfigDetails object
	"""
	availability_zone_count: Optional[Integer]
class AwsOpenSearchServiceDomainClusterConfigDetails(ASFFBaseModel):
	"""
Details about the configuration of an OpenSearch cluster.

:param instance_count: The number of data nodes to use in the OpenSearch domain.
:param warm_enabled: Whether UltraWarm is enabled.
:param warm_count: The number of UltraWarm instances.
:param dedicated_master_enabled: Whether to use a dedicated master node for the OpenSearch domain. A dedicated master node performs cluster management tasks, but does not hold data or respond to data upload requests.
:param zone_awareness_config: Configuration options for zone awareness. Provided if ZoneAwarenessEnabled is true.
:param dedicated_master_count: The number of instances to use for the master node. If this attribute is specified, then DedicatedMasterEnabled must be true.
:param instance_type: The instance type for your data nodes. 
:param warm_type: The type of UltraWarm instance.
:param zone_awareness_enabled: Whether to enable zone awareness for the OpenSearch domain. When zone awareness is enabled, OpenSearch Service allocates the cluster's nodes and replica index shards across Availability Zones (AZs) in the same Region. This prevents data loss and minimizes downtime if a node or data center fails.
:param dedicated_master_type: The hardware configuration of the computer that hosts the dedicated master node. If this attribute is specified, then DedicatedMasterEnabled must be true. 

:return: AwsOpenSearchServiceDomainClusterConfigDetails object
	"""
	instance_count: Optional[Integer]
	warm_enabled: Optional[Boolean]
	warm_count: Optional[Integer]
	dedicated_master_enabled: Optional[Boolean]
	zone_awareness_config: Optional[AwsOpenSearchServiceDomainClusterConfigZoneAwarenessConfigDetails]
	dedicated_master_count: Optional[Integer]
	instance_type: Optional[NonEmptyString]
	warm_type: Optional[NonEmptyString]
	zone_awareness_enabled: Optional[Boolean]
	dedicated_master_type: Optional[NonEmptyString]
class AwsOpenSearchServiceDomainDomainEndpointOptionsDetails(ASFFBaseModel):
	"""
Information about additional options for the domain endpoint.

:param custom_endpoint_certificate_arn: The ARN for the security certificate. The certificate is managed in ACM.
:param custom_endpoint_enabled: Whether to enable a custom endpoint for the domain.
:param enforce_https: Whether to require that all traffic to the domain arrive over HTTPS.
:param custom_endpoint: The fully qualified URL for the custom endpoint.
:param tls_security_policy: The TLS security policy to apply to the HTTPS endpoint of the OpenSearch domain.

:return: AwsOpenSearchServiceDomainDomainEndpointOptionsDetails object
	"""
	custom_endpoint_certificate_arn: Optional[NonEmptyString]
	custom_endpoint_enabled: Optional[Boolean]
	enforce_https: Optional[Boolean]
	custom_endpoint: Optional[NonEmptyString]
	tls_security_policy: Optional[NonEmptyString]
class AwsOpenSearchServiceDomainVpcOptionsDetails(ASFFBaseModel):
	"""
Contains information that OpenSearch Service derives based on the VPCOptions for the domain.

:param security_group_ids: The list of security group IDs that are associated with the VPC endpoints for the domain.
:param subnet_ids: A list of subnet IDs that are associated with the VPC endpoints for the domain.

:return: AwsOpenSearchServiceDomainVpcOptionsDetails object
	"""
	security_group_ids: Optional[NonEmptyStringList]
	subnet_ids: Optional[NonEmptyStringList]
class AwsOpenSearchServiceDomainLogPublishingOption(ASFFBaseModel):
	"""
...
:return: AwsOpenSearchServiceDomainLogPublishingOption object
	"""
	cloud_watch_logs_log_group_arn: Optional[NonEmptyString]
	enabled: Optional[Boolean]
class AwsOpenSearchServiceDomainLogPublishingOptionsDetails(ASFFBaseModel):
	"""
Configures the CloudWatch Logs to publish for the OpenSearch domain.

:param index_slow_logs: Configures the OpenSearch index logs publishing.
:param search_slow_logs: Configures the OpenSearch search slow log publishing.
:param audit_logs: Configures the OpenSearch audit logs publishing.

:return: AwsOpenSearchServiceDomainLogPublishingOptionsDetails object
	"""
	index_slow_logs: Optional[AwsOpenSearchServiceDomainLogPublishingOption]
	search_slow_logs: Optional[AwsOpenSearchServiceDomainLogPublishingOption]
	audit_logs: Optional[AwsOpenSearchServiceDomainLogPublishingOption]
class AwsOpenSearchServiceDomainDetails(ASFFBaseModel):
	"""
Information about an Amazon OpenSearch Service domain.

:param arn: The ARN of the OpenSearch Service domain.
:param access_policies: IAM policy document that specifies the access policies for the OpenSearch Service domain.
:param domain_name: The name of the endpoint.
:param id: The identifier of the domain.
:param domain_endpoint: The domain endpoint.
:param engine_version: The version of the domain engine.
:param encryption_at_rest_options: Details about the configuration for encryption at rest.
:param node_to_node_encryption_options: Details about the configuration for node-to-node encryption.
:param service_software_options: Information about the status of a domain relative to the latest service software.
:param cluster_config: Details about the configuration of an OpenSearch cluster.
:param domain_endpoint_options: Additional options for the domain endpoint.
:param vpc_options: Information that OpenSearch Service derives based on VPCOptions for the domain.
:param log_publishing_options: Configures the CloudWatch Logs to publish for the OpenSearch domain.
:param domain_endpoints: The domain endpoints. Used if the OpenSearch domain resides in a VPC. This is a map of key-value pairs. The key is always vpc. The value is the endpoint.

:return: AwsOpenSearchServiceDomainDetails object
	"""
	arn: Optional[NonEmptyString]
	access_policies: Optional[NonEmptyString]
	domain_name: Optional[NonEmptyString]
	id: Optional[NonEmptyString]
	domain_endpoint: Optional[NonEmptyString]
	engine_version: Optional[NonEmptyString]
	encryption_at_rest_options: Optional[AwsOpenSearchServiceDomainEncryptionAtRestOptionsDetails]
	node_to_node_encryption_options: Optional[AwsOpenSearchServiceDomainNodeToNodeEncryptionOptionsDetails]
	service_software_options: Optional[AwsOpenSearchServiceDomainServiceSoftwareOptionsDetails]
	cluster_config: Optional[AwsOpenSearchServiceDomainClusterConfigDetails]
	domain_endpoint_options: Optional[AwsOpenSearchServiceDomainDomainEndpointOptionsDetails]
	vpc_options: Optional[AwsOpenSearchServiceDomainVpcOptionsDetails]
	log_publishing_options: Optional[AwsOpenSearchServiceDomainLogPublishingOptionsDetails]
	domain_endpoints: Optional[FieldMap]
class AwsEc2VpcEndpointServiceServiceTypeDetails(ASFFBaseModel):
	"""
The service type information for a VPC endpoint service.

:param service_type: The type of service.

:return: AwsEc2VpcEndpointServiceServiceTypeDetails object
	"""
	service_type: Optional[NonEmptyString]
AwsEc2VpcEndpointServiceServiceTypeList = List[AwsEc2VpcEndpointServiceServiceTypeDetails]
class AwsEc2VpcEndpointServiceDetails(ASFFBaseModel):
	"""
Contains details about the service configuration for a VPC endpoint service.

:param acceptance_required: Whether requests from other Amazon Web Services accounts to create an endpoint to the service must first be accepted.
:param availability_zones: The Availability Zones where the service is available.
:param base_endpoint_dns_names: The DNS names for the service.
:param manages_vpc_endpoints: Whether the service manages its VPC endpoints.
:param gateway_load_balancer_arns: The ARNs of the Gateway Load Balancers for the service.
:param network_load_balancer_arns: The ARNs of the Network Load Balancers for the service.
:param private_dns_name: The private DNS name for the service.
:param service_id: The identifier of the service.
:param service_name: The name of the service.
:param service_state: The current state of the service.
:param service_type: The types for the service.

:return: AwsEc2VpcEndpointServiceDetails object
	"""
	acceptance_required: Optional[Boolean]
	availability_zones: Optional[NonEmptyStringList]
	base_endpoint_dns_names: Optional[NonEmptyStringList]
	manages_vpc_endpoints: Optional[Boolean]
	gateway_load_balancer_arns: Optional[NonEmptyStringList]
	network_load_balancer_arns: Optional[NonEmptyStringList]
	private_dns_name: Optional[NonEmptyString]
	service_id: Optional[NonEmptyString]
	service_name: Optional[NonEmptyString]
	service_state: Optional[NonEmptyString]
	service_type: Optional[AwsEc2VpcEndpointServiceServiceTypeList]
class AwsXrayEncryptionConfigDetails(ASFFBaseModel):
	"""
Information about the encryption configuration for X-Ray.

:param key_id: The identifier of the KMS key that is used for encryption. Provided if Type is KMS.
:param status: The current status of the encryption configuration. When Status is UPDATING, X-Ray might use both the old and new encryption.
:param type: The type of encryption. KMS indicates that the encryption uses KMS keys. NONE indicates to use the default encryption.

:return: AwsXrayEncryptionConfigDetails object
	"""
	key_id: Optional[NonEmptyString]
	status: Optional[NonEmptyString]
	type: Optional[NonEmptyString]
class AwsWafRateBasedRuleMatchPredicate(ASFFBaseModel):
	"""
A match predicate. A predicate might look for characteristics such as specific IP addresses, geographic locations, or sizes.

:param data_id: The unique identifier for the predicate.
:param negated: If set to true, then the rule actions are performed on requests that match the predicate settings. If set to false, then the rule actions are performed on all requests except those that match the predicate settings. 
:param type: The type of predicate.

:return: AwsWafRateBasedRuleMatchPredicate object
	"""
	data_id: Optional[NonEmptyString]
	negated: Optional[Boolean]
	type: Optional[NonEmptyString]
AwsWafRateBasedRuleMatchPredicateList = List[AwsWafRateBasedRuleMatchPredicate]
class AwsWafRateBasedRuleDetails(ASFFBaseModel):
	"""
...
:param metric_name: The name of the metrics for the rate-based rule.
:param name: The name of the rate-based rule.
:param rate_key: The field that WAF uses to determine whether requests are likely arriving from single source and are subject to rate monitoring.
:param rate_limit: The maximum number of requests that have an identical value for the field specified in RateKey that are allowed within a five-minute period. If the number of requests exceeds RateLimit and the other predicates specified in the rule are met, WAF triggers the action for the rule.
:param rule_id: The unique identifier for the rate-based rule.
:param match_predicates: The predicates to include in the rate-based rule.

:return: AwsWafRateBasedRuleDetails object
	"""
	metric_name: Optional[NonEmptyString]
	name: Optional[NonEmptyString]
	rate_key: Optional[NonEmptyString]
	rate_limit: Optional[Long]
	rule_id: Optional[NonEmptyString]
	match_predicates: Optional[AwsWafRateBasedRuleMatchPredicateList]
class AwsWafRegionalRateBasedRuleMatchPredicate(ASFFBaseModel):
	"""
...
:return: AwsWafRegionalRateBasedRuleMatchPredicate object
	"""
	data_id: Optional[NonEmptyString]
	negated: Optional[Boolean]
	type: Optional[NonEmptyString]
AwsWafRegionalRateBasedRuleMatchPredicateList = List[AwsWafRegionalRateBasedRuleMatchPredicate]
class AwsWafRegionalRateBasedRuleDetails(ASFFBaseModel):
	"""
...
:return: AwsWafRegionalRateBasedRuleDetails object
	"""
	metric_name: Optional[NonEmptyString]
	name: Optional[NonEmptyString]
	rate_key: Optional[NonEmptyString]
	rate_limit: Optional[Long]
	rule_id: Optional[NonEmptyString]
	match_predicates: Optional[AwsWafRegionalRateBasedRuleMatchPredicateList]
class AwsEcrRepositoryImageScanningConfigurationDetails(ASFFBaseModel):
	"""
The image scanning configuration for a repository.

:param scan_on_push: Whether to scan images after they are pushed to a repository.

:return: AwsEcrRepositoryImageScanningConfigurationDetails object
	"""
	scan_on_push: Optional[Boolean]
class AwsEcrRepositoryLifecyclePolicyDetails(ASFFBaseModel):
	"""
Information about the lifecycle policy for the repository.

:param lifecycle_policy_text: The text of the lifecycle policy.
:param registry_id: The Amazon Web Services account identifier that is associated with the registry that contains the repository.

:return: AwsEcrRepositoryLifecyclePolicyDetails object
	"""
	lifecycle_policy_text: Optional[NonEmptyString]
	registry_id: Optional[NonEmptyString]
class AwsEcrRepositoryDetails(ASFFBaseModel):
	"""
Provides information about an Amazon Elastic Container Registry repository.

:param arn: The ARN of the repository.
:param image_scanning_configuration: The image scanning configuration for a repository.
:param image_tag_mutability: The tag mutability setting for the repository.
:param lifecycle_policy: Information about the lifecycle policy for the repository.
:param repository_name: The name of the repository.
:param repository_policy_text: The text of the repository policy.

:return: AwsEcrRepositoryDetails object
	"""
	arn: Optional[NonEmptyString]
	image_scanning_configuration: Optional[AwsEcrRepositoryImageScanningConfigurationDetails]
	image_tag_mutability: Optional[NonEmptyString]
	lifecycle_policy: Optional[AwsEcrRepositoryLifecyclePolicyDetails]
	repository_name: Optional[NonEmptyString]
	repository_policy_text: Optional[NonEmptyString]
class AwsEksClusterResourcesVpcConfigDetails(ASFFBaseModel):
	"""
Information about the VPC configuration used by the cluster control plane.

:param security_group_ids: The security groups that are associated with the cross-account elastic network interfaces that are used to allow communication between your nodes and the Amazon EKS control plane.
:param subnet_ids: The subnets that are associated with the cluster.

:return: AwsEksClusterResourcesVpcConfigDetails object
	"""
	security_group_ids: Optional[NonEmptyStringList]
	subnet_ids: Optional[NonEmptyStringList]
class AwsEksClusterLoggingClusterLoggingDetails(ASFFBaseModel):
	"""
Details for a cluster logging configuration.

:param enabled: Whether the logging types that are listed in Types are enabled.
:param types: A list of logging types.

:return: AwsEksClusterLoggingClusterLoggingDetails object
	"""
	enabled: Optional[Boolean]
	types: Optional[NonEmptyStringList]
AwsEksClusterLoggingClusterLoggingList = List[AwsEksClusterLoggingClusterLoggingDetails]
class AwsEksClusterLoggingDetails(ASFFBaseModel):
	"""
The logging configuration for an Amazon EKS cluster.

:param cluster_logging: Cluster logging configurations.

:return: AwsEksClusterLoggingDetails object
	"""
	cluster_logging: Optional[AwsEksClusterLoggingClusterLoggingList]
class AwsEksClusterDetails(ASFFBaseModel):
	"""
Provides details about an Amazon EKS cluster.

:param arn: The ARN of the cluster.
:param certificate_authority_data: The certificate authority data for the cluster.
:param cluster_status: The status of the cluster.
:param endpoint: The endpoint for the Amazon EKS API server.
:param name: The name of the cluster.
:param resources_vpc_config: The VPC configuration used by the cluster control plane.
:param role_arn: The ARN of the IAM role that provides permissions for the Amazon EKS control plane to make calls to Amazon Web Services API operations on your behalf.
:param version: The Amazon EKS server version for the cluster.
:param logging: The logging configuration for the cluster.

:return: AwsEksClusterDetails object
	"""
	arn: Optional[NonEmptyString]
	certificate_authority_data: Optional[NonEmptyString]
	cluster_status: Optional[NonEmptyString]
	endpoint: Optional[NonEmptyString]
	name: Optional[NonEmptyString]
	resources_vpc_config: Optional[AwsEksClusterResourcesVpcConfigDetails]
	role_arn: Optional[NonEmptyString]
	version: Optional[NonEmptyString]
	logging: Optional[AwsEksClusterLoggingDetails]
...
:param aws_open_search_service_domain: Details about an Amazon OpenSearch Service domain.
:param aws_ec2_vpc_endpoint_service: Details about the service configuration for a VPC endpoint service.
:param aws_xray_encryption_config: Information about the encryption configuration for X-Ray.
:param aws_waf_rate_based_rule: Details about a rate-based rule for global resources.
:param aws_waf_regional_rate_based_rule: Details about a rate-based rule for Regional resources.
:param aws_ecr_repository: Information about an Amazon Elastic Container Registry repository.
:param aws_eks_cluster: Details about an Amazon EKS cluster.
...
:return: ResourceDetails object
	...
	aws_open_search_service_domain: Optional[AwsOpenSearchServiceDomainDetails]
	aws_ec2_vpc_endpoint_service: Optional[AwsEc2VpcEndpointServiceDetails]
	aws_xray_encryption_config: Optional[AwsXrayEncryptionConfigDetails]
	aws_waf_rate_based_rule: Optional[AwsWafRateBasedRuleDetails]
	aws_waf_regional_rate_based_rule: Optional[AwsWafRegionalRateBasedRuleDetails]
	aws_ecr_repository: Optional[AwsEcrRepositoryDetails]
	aws_eks_cluster: Optional[AwsEksClusterDetails]
